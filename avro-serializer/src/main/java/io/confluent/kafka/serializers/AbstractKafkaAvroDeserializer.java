...
public abstract class AbstractKafkaAvroDeserializer extends AbstractKafkaAvroSerDe {
  ...
  private final Map<Schema, Schema> targetSchemaCache = new ConcurrentHashMap<Schema, Schema>();
  ...
  private Schema getReaderSchema(Schema writerSchema) {
    Schema readerSchema = targetSchemaCache.get(writerSchema);
    if (readerSchema == null) {
      Class targetClass = SpecificData.get().getClass(writerSchema);
      if (targetClass != null) {
        try {
          readerSchema = ((SpecificRecord) targetClass.newInstance()).getSchema();
        } catch (InstantiationException e) {
          throw new SerializationException(writerSchema.getFullName() + " specified by the " +
            "writers schema is not a SpecificRecord which is required to find the readers schema.");
        } catch (IllegalAccessException e) {
          throw new SerializationException(writerSchema.getFullName() + " specified by the " +
            "writers schema is not a SpecificRecord which is required to find the readers schema.");
        }
        targetSchemaCache.put(writerSchema, readerSchema);
      } else {
